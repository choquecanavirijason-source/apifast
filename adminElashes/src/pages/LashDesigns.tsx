...
import { 
  Plus, Search, Edit, Trash2, Image as ImageIcon, X, Brush, AlertTriangle 
} from "lucide-react";
...
export default function LashDesignsPage() {
  ...
  const loadDesigns = async () => {
    ...
    try {
      ...
      toast.error("Error al cargar disenos");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit = (design: LashDesign) => {
    setCurrentDesign(design);
    setForm({ name: design.name, image: design.image });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      if (currentDesign) {
        await api.put(`/catalogs/lash-designs/${currentDesign.id}`, {
          name,
          image: form.image || null,
        });
      } else {
        await api.post("/catalogs/lash-designs", {
          name,
          image: form.image || null,
        });
      }
      ...
      toast.error(getErrorMessage(error, "No se pudo guardar el diseno"));
    } finally {
      setSaving(false);
    }
  };
  ...
  const openDeleteDialog = (design: LashDesign) => {
    setDialogConfig({
      isOpen: true,
      title: "¿Eliminar diseño?",
      message: `Estás a punto de eliminar el diseño "${design.name}". Esta acción no se puede deshacer.`,
      variant: "danger",
      onConfirm: () => handleDelete(design),
    });
  };

  const handleDelete = async (design: LashDesign) => {
    setIsProcessing(true);
    try {
      await api.delete(`/catalogs/lash-designs/${design.id}`);
      await loadDesigns();
      setDialogConfig(null);
    } catch (error) {
      toast.error(getErrorMessage(error, "No se pudo eliminar el diseno"));
    } finally {
  ...
  return (
    <div className="p-6 md:p-10 min-h-screen bg-slate-50/50 font-sans">
      ...
      {/* Encabezado con Icono de Pincel/Diseño */}
      <div className="mb-8 flex items-center gap-3">
        <div className="p-2 bg-emerald-100 rounded-lg">
            <Brush className="w-6 h-6 text-[#094732]" />
        </div>
        <div>
            <h1 className="text-2xl font-bold text-slate-800">Tecnologia</h1>
            <p className="text-slate-500 text-sm">Catálogo de estilos de mapeo (Mapping)</p>
        </div>
      </div>
      ...
      <div className="bg-emerald-50/40 rounded-[1.5rem] border border-emerald-100 shadow-sm p-6 md:p-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          ...
          {!loading && filtered.length === 0 && (
            ...
            >
                {/* Imagen */}
                <div className="relative aspect-square w-full rounded-xl overflow-hidden bg-slate-50 border border-slate-100 flex items-center justify-center group-hover:scale-[1.02] transition-transform duration-500">
                    {item.image ? (
                        <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                    ) : (
                        <div className="flex flex-col items-center gap-2 text-slate-300">
                            <Brush className="w-10 h-10 opacity-50" />
                            <span className="text-xs font-medium">Sin esquema</span>
                        </div>
                    )}
                </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            <div>
              ...
              <input
                ...
                placeholder="Ej. Ojo de Gato"
                required
              />
            </div>

            <div>
              <label className="block text-sm font-bold text-slate-700 mb-2">Esquema / Imagen</label>
                  ...
                  {form.image ? (
                     <div className="relative h-40 mx-auto rounded-lg overflow-hidden w-fit">
                      ...
                      <div className="py-8 flex flex-col items-center text-slate-400">
                          ...
                          <span className="text-sm">Click para subir esquema</span>
                      </div>
                  )}
              </div>
              {form.image && (
                ...
                >
                  <Trash2 className="w-3 h-3" /> Eliminar imagen actual
                </button>
              )}
            </div>
