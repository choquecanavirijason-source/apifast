...
import { 
  Plus, Search, Edit, Trash2, Image as ImageIcon, X, Sparkles, AlertTriangle 
} from "lucide-react";
...
export default function EffectsPage() {
  ...
  const openEdit = (effect: Effect) => {
    setCurrentEffect(effect);
    setForm({ name: effect.name, image: effect.image });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      if (currentEffect) {
        await api.put(`/catalogs/effects/${currentEffect.id}`, {
          name,
          image: form.image || null,
        });
      } else {
        await api.post("/catalogs/effects", {
          name,
          image: form.image || null,
        });
      }
  ...
  const openDeleteDialog = (effect: Effect) => {
    setDialogConfig({
      isOpen: true,
      title: "¿Eliminar efecto?",
      message: `Estás a punto de eliminar "${effect.name}". Esta acción no se puede deshacer.`,
      variant: "danger",
      onConfirm: () => handleDelete(effect),
    });
  };
  ...
  const handleDelete = async (effect: Effect) => {
    ...
    try {
      await api.delete(`/catalogs/effects/${effect.id}`);
      await loadEffects();
  ...
  return (
    <div className="p-6 md:p-10 min-h-screen bg-slate-50/50 font-sans">
      ...
      <div className="mb-8 flex items-center gap-3">
        ...
        <div>
            <h1 className="text-2xl font-bold text-slate-800">Catálogo de Efectos</h1>
            <p className="text-slate-500 text-sm">Gestiona los estilos de pestañas disponibles</p>
        </div>
      </div>
      ...
      <div className="bg-emerald-50/40 rounded-[1.5rem] border border-emerald-100 shadow-sm p-6 md:p-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          ...
          {!loading && filtered.length === 0 && (
                <div className="col-span-full py-12 text-center text-slate-400 bg-white/50 rounded-2xl border border-dashed border-slate-300">
                    No se encontraron efectos con ese nombre.
                </div>
            )}
            ...
            >
                ...
                <div className="flex items-center justify-between mt-auto">
                    <span className="font-bold text-slate-700 text-lg">{effect.name}</span>
                    ...
                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                        ...
                        <button
                            onClick={() => openDeleteDialog(effect)}
      ...
      <SimpleModal
        isOpen={isModalOpen}
        onClose={closeModal}
        title={currentEffect ? "Editar Efecto" : "Crear Nuevo Efecto"}
      >
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-2">Nombre del Efecto</label>
              <input
                name="name"
                value={form.name}
