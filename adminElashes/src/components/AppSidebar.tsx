...
export default function AppSidebar({ collapsed }: { collapsed: boolean }) {
  ...
  return (
    <aside
      className={`relative h-screen flex flex-col transition-all duration-300 border-r border-emerald-900/30 shrink-0 ${
        collapsed ? "w-20" : "w-64"
      }`}
    ...
    >
      ...
      <div className="h-20 flex items-center px-6 border-b border-emerald-800/30 shrink-0">
        {!collapsed && <span className="text-2xl font-black text-white tracking-tight">FaceMapping Studio</span>}
        {collapsed && <span className="mx-auto text-xl font-black text-emerald-400">FM</span>}
      </div>
